...
$("#addproduct").on("click", function () {
  ...
  var data =
    ...
    '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' +
    functionNum +
    '), billUpyog()" autocomplete="off" inputmode="numeric"></td><td> <input type="text" class="form-control vat" name="product_tax[]" id="vat-' +
    cvalue +
    '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' +
    functionNum +
...
// Added by ashfaq for entering direct total amount
var manualTotal = function (numb) {
  var enteredTotal = accounting.unformat(
    $("#manualTotal-" + numb).val(),
    accounting.settings.number.decimal,
  );
  if (enteredTotal) {
    $("#price-" + numb).attr('readonly', true);
    var totalItemDiscount = accounting.unformat(
      $("#discount-" + numb).val(),
      accounting.settings.number.decimal,
    );
    var qty = accounting.unformat(
      $("#amount-" + numb).val(),
      accounting.settings.number.decimal,
    );
    var newrate = (enteredTotal + totalItemDiscount) / qty;
    $("#price-" + numb).val(accounting.formatNumber(newrate));
  } else {
    $("#price-" + numb).attr('readonly', false);
  }
};
...
var rowTotal = function (numb) {
  ...
  var taxo = 0;
  var disco = 0;
  var totalPrice = enteredTotal
    ? enteredTotal
    : amountVal.toFixed(two_fixed) * priceVal;
  var tax_status = $("#taxformat option:selected").val();
  var disFormat = $("#discount_format").val();
  ...
  var totalQty = 0;
  qty.forEach((element) => {
    totalQty = Number(totalQty) + Number(element.value);
  });
  if (document.getElementById("totalQty"))
    document.getElementById("totalQty").innerHTML = Number(totalQty);
  // end -----
...
$(document).on("click", ".select_pos_item", function (e) {
  ...
  // $('.pdIn').each(function () {
  //     console.log('in custom js val', $(this).val())
  //     if (pid == $(this).val()) {
  ...
  if (flag) {
    ...
    var functionNum = "'" + cvalue + "'";
    var productNameDescSeparator = $(this).attr("data-pcode") ? "-" : "";
    count = $("#saman-row div").length;
    var data =
      '<tr id="ppid-' +
      cvalue +
      ...
      "</span> <strong><span class='ttlText' id=\"result-" +
      cvalue +
      '">0</span></strong><input type="text" class="form-control p-width p-mobile req" name="manualTotal[]"  inputmode="numeric" id="manualTotal-' +
      cvalue +
      '" onkeypress="return isNumber(event)" onkeyup="rowTotal(' +
      functionNum +
      '), billUpyog()" autocomplete="off" >' +
      '</td> <td class="text-center"><button type="button" data-rowid="' +
      cvalue +
