...
//search product in invoice
	...
	public function pos_c_search()
	{
		...
		if ($name) {
			$query = $this->db->query("SELECT id,name,phone,discount_c,address FROM geopos_customers WHERE $whr (UPPER(name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(phone)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
			$result = $query->result_array();
			echo '<ol>';
			$i = 1;
			foreach ($result as $row) {
	...
	public function pos_search()
	{
		...
		if ($billing_settings['key1'] == 2) {
            ...
            $query = "SELECT geopos_products.*,geopos_product_serials.serial FROM geopos_product_serials  LEFT JOIN geopos_products  ON geopos_products.pid=geopos_product_serials.product_id $join WHERE " . $qw . "geopos_product_serials.serial LIKE '" . strtoupper($name) . "%' LIMIT 32";


        } else {
            $query = "SELECT geopos_products.* $e FROM geopos_products $join WHERE " . $qw . "(UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%' $bar OR geopos_products.product_code LIKE '" . strtoupper($name) . "%') ORDER BY geopos_products.qty DESC LIMIT 32";
		...
		foreach ($result as $row) {
            ...
            if($row['qty'] > 0){
                $out .= '    <div class="col-3 border mb-1 ">';
            } else {
                $out .= '    <div class="col-3 border mb-1 " style="
                background: #cacbcc;
                pointer-events: none;
            "> <span style="color:#FF0000">Out of stock</span>';
            }

            $out .= '   <div class="rounded p-item">
	...
	public function group_pos_search()
	{
        ...
        if (is_numeric($name)) {
            ...
            $bar = " OR (geopos_products.barcode LIKE '" . (substr($barcode, 0, -1)) . "%' OR geopos_products.barcode LIKE '" . $name . "%')";
            //  $query = "SELECT geopos_products.* FROM geopos_products $join WHERE " . $qw . " $bar LIMIT 32";
        }
        if ($billing_settings['key1'] == 2) {

            $query = "SELECT geopos_products.*,geopos_product_serials.serial FROM geopos_product_serials  LEFT JOIN geopos_products  ON geopos_products.pid=geopos_product_serials.product_id $join WHERE " . $qw . "geopos_product_serials.serial LIKE '" . strtoupper($name) . "%'  LIMIT 18";

        } else {
            $query = "SELECT geopos_products.* $e FROM geopos_products $join WHERE " . $qw . "(UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%' $bar OR geopos_products.product_code LIKE '" . strtoupper($name) . "%') ORDER BY geopos_products.product_name LIMIT 18";
        }
            ...
            $out .= '    <div class="col-2 border mb-1"  ><div class=" rounded" >
                                 <a  id="posp' . $i . '"  class="v2_select_pos_item round"   data-name="' . $row['product_name'] . '"  data-price="' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . amountFormat_general($row['taxrate']) . '"  data-discount="' . amountFormat_general($row['disrate']) . '" data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . amountFormat_general($row['qty']) . '" data-unit="' . $row['unit'] . '" data-serial="' . @$row['serial'] . '">
                                        ...
                                        <div class="text-center" style="margin-top: 4px;">
                                            ...
                                            <small style="white-space: pre-wrap;">' . $row['product_name'] . '</small>
                                            <br/>
                                            <small style="white-space: pre-wrap;"> Cost: ' . $row['fproduct_price'] . '</small>
                                            <br/>
                                            <small style="white-space: pre-wrap;"> MRP: ' . $row['product_price'] . '</small>
