...
class Reports extends CI_Controller
{
    ...
    function group_by($key, $data) {
        ...
        foreach($data as $val) {
            if(array_key_exists($key, $val)){
                $result[$val[$key]][] = array('tax' => $val['totaltax'], 'taxable' => $val['subtotal']);
            }
        }
    ...
    public function taxviewstatements_load()
    {
        ...
        if ($trans_type == 'Sales') {
            ...
            $query = $this->db->query("SELECT geopos_customers.taxid AS VAT_Number,geopos_invoices.tid AS invoice_number,geopos_invoices.id AS id_number,geopos_invoices.total AS amount,geopos_invoices.tax AS tax,geopos_customers.name AS customer_name,geopos_customers.company AS Company_Name,geopos_invoices.invoicedate AS date FROM geopos_invoices LEFT JOIN geopos_customers ON geopos_invoices.csd=geopos_customers.id" . $where);
        } else {

            $where = " WHERE (DATE(geopos_purchase.invoicedate) BETWEEN '$sdate' AND '$edate') ";
            if ($lid > 0) $where .= " AND (geopos_invoices.loc=$lid)";
            $query = $this->db->query("SELECT geopos_supplier.taxid AS VAT_Number,geopos_purchase.tid AS invoice_number,geopos_invoices.id AS id_number,geopos_purchase.total AS amount,geopos_purchase.tax AS tax,geopos_supplier.name AS customer_name,geopos_supplier.company AS Company_Name,geopos_purchase.invoicedate AS date FROM geopos_purchase LEFT JOIN geopos_supplier ON geopos_purchase.csd=geopos_supplier.id" . $where);
        }
...
//echo $where;
        ...
        $trimmedIds = rtrim($ids,',');
        $query = $this->db->query("SELECT * from geopos_invoice_items where tid IN (". $trimmedIds . ")");
        $products = $query->result_array();
        $taxgrp = $this->group_by('tax', $products);
        $kyy = array_keys($taxgrp); 
        echo '<tr><td colspan="6" style="padding:0;"><div style="
            font-weight: 700;
            text-align: center;
            padding: 10px 20px;
            background-color: black;
            color: #fff;
        ">TAX BREAKDOWN</div></td></tr><tr> <th></th><th></th><th></th><th> Rate</th> <th> Taxable Amount </th> <th> Tax </th> </tr>';
        foreach($kyy as $key){
            
            $row_line =  '<tr> <td></td><td></td><td></td><td>' . $key . '</td>';
            //$summ = 0;
            $taxablesumm = 0;
            ...
            if($key != 0)
                $taxablesumm = ($taxablesumm / (100 + $key)) * 100;
                
            $row_line .= '<td>' . amountExchange($taxablesumm, $invoice['multi'], $invoice['loc']) . '</td><td>' . amountExchange(($taxablesumm * $key)/100, $invoice['multi'], $invoice['loc']). '</td> </tr>';
...

    {
        ...
        $this->load->view('reports/weight', $data);
    ...
    public function weightCalc()
    {
        ...
        if ($this->input->post('check')) {
            ...
            $diff = $date2->diff($date1)->format("%a");
            if ($diff < 365) {
                $products = $this->reports->customsalesstatementbyweight($lid, $sdate, $edate, $category);
                if(count($products) == 0){
                    echo json_encode(array('status' => 'Error', 'message' => 'no records', 'param1' => '<h4>No records</h4>'));
                    return;
                }
                $html = '<table class="table"><thead><tr><th scope="col">Product Id</th><th scope="col">Product Name</th><th scope="col">Item Weight</th><th scope="col">Qty</th><th scope="col">Total Weight</th><th scope="col">Sub Total</th></tr></thead><tbody>';
                $totalWeights = 0;
                $totalQtys = 0;
                $SubTotalsAmount = 0;
                foreach ($products as $prd) {
                    $totalWeight = $prd['weight'] != null ? (double)$prd['weight'] * (double)$prd['qtys'] : (double)$prd['qtys'];
                    $totalWeights += $totalWeight;
                    $totalQtys += (double)$prd['qtys'];
                    $SubTotalsAmount += (double)$prd['subtotal'];
