...
class Pos_invoices extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $data['def_account'] = $this->invocies->defaultAccount();
    ...
    public function action()
    {
        ...
        if ($d_trans['key2']) {
           ...
           $t_data['type'] = 'Expense';
            ...
            $data = array('tid' => $invocieno, 'invoicedatetime' => $bill_invoicedatetime, 'invoicedate' => $bill_date, 'invoiceduedate' => $bill_due_date, 'subtotal' => $subtotal, 'shipping' => $shipping, 'total' => $total, 'notes' => $notes, 'csd' => $customer_id, 'eid' => $emp, 'taxstatus' => $tax, 'discstatus' => $discstatus, 'format_discount' => $discountFormat, 'refer' => $refer, 'term' => $pterms, 'multi' => $currency, 'i_class' => 1, 'loc' => $this->aauth->get_user()->loc);
            $invoice_true = false;
            if ($this->db->insert('geopos_invoices', $data)) {
                $invoice_true = true;
                $tid = $invocieno;
...
//draft
    ...
    public function ajax_list()
    {
        ...
        // All delivery boys
        $delivery_boys = $this->delivery_model->getAllDeliveryBoys();
        ...
        foreach ($list as $invoices) {
            ...
            foreach ($delivery_boys as $row) {
                ...
                if($invoices->boy_id == $row['boy_id']){
                    $selectedOption .= 'selected="selected"';
                }
                $options .= $selectedOption . '>' . $row['boy_name'] .'</option>';
            }
            $no++;
            $row = array();
            $row[] = $no;
    ...
    public function view()
    {
        ...
        // Added by Hussain GS
        // All delivery boys
        $data['delivery_boys'] = $this->delivery_model->getAllDeliveryBoys();
        // end
        // fetch delivery boy
        $data['delivery_boy'] = $this->delivery_model->get_delivery_boy($tid);
        $data['delivery_boy']['assigned'] = ($data['delivery_boy'] == null) ? 0 : 1;
        // end
        $this->load->view('fixed/header', $head);
...
echo 6;
    ...
    function group_by($key, $data) {
        ...
        foreach($data as $val) {
            if(array_key_exists($key, $val) && $val[$key] != 0){
    ...
    function thermal_pdf()
    {
        ...
        $data['taxgrp'] = $this->group_by('tax', $data['products']);
        // --------
