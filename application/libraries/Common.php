...
class Common
{
    ...
    function disclist($isPos = false)
    {
        // for pos discount types for default FLAT_BEFORE_TAX
        if($isPos == true){
            $this->PI->db->where('id', 70);
            $query = $this->PI->db->get('univarsal_api');
            $row1 = $query->row_array();
            $disclist = '<option value="' . $row1['key1'] . '">' . $row1['other'] . '</option> ';
            $disclist .= '<option value="flat">' . $this->PI->lang->line('Flat Discount') . ' ' . $this->PI->lang->line('After TAX') . '</option>
                                <option value="%">' . $this->PI->lang->line('% Discount') . ' ' . $this->PI->lang->line('After TAX') . '</option>
    ...
    function disc_status($isPos = false)
    {
        if($isPos == true){
            // for pos discount types for default FLAT_BEFORE_TAX
            $this->PI->db->where('id', 70);
