...
class Reports_model extends CI_Model
{
    ...
    public function customsalesstatementbyweight($lid, $sdate, $edate, $category)
    {
        $query = $this->db->query("SELECT geopos_products.weight,geopos_products.product_name,SUM(geopos_invoice_items.subtotal) subtotal,SUM(geopos_invoice_items.qty) qtys,geopos_invoice_items.pid,geopos_products.weight,geopos_products.product_name FROM geopos_invoice_items LEFT JOIN geopos_invoices ON geopos_invoices.id=geopos_invoice_items.tid LEFT JOIN geopos_products ON geopos_products.pid=geopos_invoice_items.pid LEFT JOIN geopos_product_cat ON geopos_product_cat.id=geopos_products.pcat WHERE geopos_invoices.status!='canceled' AND geopos_invoices.loc='$lid' AND (DATE(geopos_invoices.invoicedate) BETWEEN DATE('$sdate') AND DATE('$edate')) AND geopos_products.pcat='$category' GROUP BY geopos_invoice_items.pid");
        $result = $query->result_array();
        return $result;
    }
